...
if _IS_WINDOWS:
    ...
    def _get_cpu_pct_win():
        ...
        d_busy = busy - _prev_idle
        _prev_idle = idle_t
        _prev_total = total
        if d_total == 0:
            return 0.0
        return max(0.0, min(100.0, d_busy * 100.0 / d_total))

    def _get_mem_info_win():
        mem = _MEMORYSTATUSEX()
        mem.dwLength = ctypes.sizeof(_MEMORYSTATUSEX)
        ctypes.windll.kernel32.GlobalMemoryStatusEx(ctypes.byref(mem))
        used = mem.ullTotalPhys - mem.ullAvailPhys
        return used, mem.ullTotalPhys, mem.dwMemoryLoad

    def _get_disk_info_win():
    ...
    def _get_net_info_win():
        global _net_prev_rx, _net_prev_tx
        try:
            import subprocess
            out = subprocess.check_output(
                ["netstat", "-e"], stderr=subprocess.DEVNULL, text=True)
            rx = tx = 0
            for line in out.splitlines():
                ...
                if "Received" in line:
                    ...
                    tx = int(line.split()[-1])
            d_rx = max(0, rx - _net_prev_rx)
            d_tx = max(0, tx - _net_prev_tx)
            _net_prev_rx = rx
            _net_prev_tx = tx
            return d_rx // 1024, d_tx // 1024
        except Exception:
            return 0, 0
    ...
    def _sys_uptime_win():
        try:
            ...
            secs = kernel32.GetTickCount64() // 1000
            h = secs // 3600
            m = (secs % 3600) // 60
            s = secs % 60
    ...
    def _load_avg_win():
        cpu = _get_cpu_pct_win()
        return f"{cpu:.0f}", "-", "-"


def _get_cpu_pct():
    if _IS_WINDOWS:
        return _get_cpu_pct_win()
    try:
        def _read():
...
def draw_dashboard(win, state):
    ...
    if paused and state.running:
        ...
        safe_addstr(win, y0 - 1, 2,
            " ! AUTO-PAUSE ACTIVE - Daily PnL da dat nguong - Bot tam dung lenh moi [R to resume] ",
