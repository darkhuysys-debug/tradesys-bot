...
def draw_update_manager(win, state, max_y, max_x, y0, local_ver, remote_ver, files, checking, error, progress, log, available, done, downloading):
    ...
    title_len = 15
    safe_addstr(win, y0, 2 + title_len, "─" * (max_x - 4 - title_len), cg(GREEN))
    ...
    safe_addstr(win, status_y, 0, "─" * max_x, cg(GREEN))
    ...
    btn_y = max_y - 2
    if done:
        safe_addstr(win, btn_y, 2, "Update complete! Closing screen...", cg(GREEN) | curses.A_BOLD)
    else:
        bx = 2
...
def draw_file_browser(win, state, max_y, max_x, y0):
    ...
    title_len = 18
    safe_addstr(win, y0, 2 + title_len, "─" * (max_x - 4 - title_len), cg(CYAN))
    ...
    safe_addstr(win, info_y, 0, "─" * max_x, cg(CYAN))
    safe_addstr(win, info_y + 1, 2, f"Total: {total} items  |  Dirs: {dirs}  |  Python: {py_files}", cg(DIM))
    safe_addstr(win, info_y + 1, max_x - 22, f"[↑↓] move  [R]refresh  [B]back", cg(DIM))
...
def draw_guide_screen(win, state):
    ...
    safe_addstr(win, info_y + 1, max_x - 22, f"[↑↓] move  [B]back", cg(DIM))
