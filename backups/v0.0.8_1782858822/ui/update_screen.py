...
def draw_update_manager(win, state, max_y, max_x, y0, local_ver, remote_ver, files, checking, error, progress, log, available, done, downloading):
    _s(win, y0, 2, "UPDATE MANAGER", cg(GREEN) | curses.A_BOLD)
    title_len = 15
    _s(win, y0, 2 + title_len, "─" * max(1, max_x - 2 - title_len), cg(GREEN))

    info_y = y0 + 1
    _s(win, info_y, 2, "Local: ", cg(WHITE))
    ...
    _s(win, status_y, 0, "─" * max(1, max_x), cg(GREEN))
    ...
    btn_y = max_y - 3
    if done:
        _s(win, btn_y, 2, "Update complete! Closing screen...", cg(GREEN) | curses.A_BOLD)
    else:
        bx = 2
...
def draw_file_browser(win, state, max_y, max_x, y0):
    ...
    title_len = 18
    _s(win, y0, 2 + title_len, "─" * max(1, max_x - 2 - title_len), cg(CYAN))
    ...
    if not file_rows:
        ...
        safe_addstr(win, list_y, name_x, hdr_line, cg(CYAN) | curses.A_BOLD)
        ...
        for i in range(max_visible):
            ...
            is_sel = (idx == sel)
            if is_sel:
                safe_addstr(win, row_y, 0, " " * max_x, cg(CYAN) | curses.A_REVERSE)
            left, tag, size, desc = _format_row(row, max_x)
            text_color = row["color"] if not is_sel else WHITE
            safe_addstr(win, row_y, name_x, left, cg(text_color) | (curses.A_BOLD if is_sel else 0))
...
def draw_guide_screen(win, state):
    ...
    hdr = (
        f"{'FILE':<{_GUIDE_FILE_W}}"
        f"{'MODULE':>{_GUIDE_MODULE_W}}"
        f" {'TAB':>{_GUIDE_TAB_W}}"
        f" {'MUC DICH':<{_GUIDE_DESC_W}}"
        f" {'KHI NAO SUA':<{_GUIDE_WHEN_W}}"
    )
    draw_box(win, list_y, 0, list_h, max_x, "UPGRADE & MODIFICATION GUIDE", CYAN)

    safe_addstr(win, list_y, name_x, hdr, cg(CYAN) | curses.A_BOLD)
    ...
    for i in range(max_visible):
        ...
        safe_addstr(win, row_y, name_x, f.ljust(_GUIDE_FILE_W), cg(WHITE if not is_sel else CYAN | curses.A_BOLD))
        safe_addstr(win, row_y, mod_x, m.rjust(_GUIDE_MODULE_W), cg(DIM if not is_sel else CYAN | curses.A_BOLD))
        safe_addstr(win, row_y, tab_x, t.rjust(_GUIDE_TAB_W), cg(YELLOW if not is_sel else CYAN | curses.A_BOLD))
        safe_addstr(win, row_y, desc_x, d.ljust(_GUIDE_DESC_W), cg(GREEN if not is_sel else WHITE | curses.A_BOLD))
        if max_when > 2:
            safe_addstr(win, row_y, when_x, w.ljust(min(_GUIDE_WHEN_W, max_when)), cg(DIM if not is_sel else CYAN | curses.A_BOLD))
    ...
    _s(win, info_y + 1, max(2, max_x - 16), "[↑↓] move  [B]back", cg(DIM))
